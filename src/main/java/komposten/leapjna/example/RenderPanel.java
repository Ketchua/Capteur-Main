...
class RenderPanel extends JPanel
{
	...
	private void drawTrackingInfo(Graphics2D g2d)
	{
		...
		for (int i = 0; i < data.nHands; i++)
		{
			...
			float posX = hand.palm.position.x;
			float posY = hand.palm.position.y;
			float posZ = hand.palm.position.z;
			float lineHeight = 20;
			...
			y += lineHeight;
			g2d.drawString(String.format("Position (x): %.02f", posX), 10, y);
			y += lineHeight;
			g2d.drawString(String.format("Position (y): %.02f", posY), 10, y);
			y += lineHeight;
			g2d.drawString(String.format("Position(z): %.02f", posZ), 10, y);
			y += lineHeight * 2;
